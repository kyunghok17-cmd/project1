...
# 世代別キーワード定義（40代以下重視版：genz 45, millennial 40, genx以上 20）
KEYWORDS = {
    'jp': {
        'genz': [
            # SNS・動画 (10)
            'TikTok', 'YouTube Shorts', 'VTuber', 'にじさんじ', 'ホロライブ', 'Discord',
            'Instagram', 'BeReal', 'Threads', 'ライブ配信',
            # ゲーム (10)
            '原神', 'ブルーアーカイブ', 'Valorant', 'Apex Legends', 'スプラトゥーン3', 'eスポーツ',
            'ポケモン', 'ゼルダの伝説', 'FF16', 'ストリートファイター6',
            # 音楽・エンタメ (8)
            ...
            # ライフスタイル (5)
            'MBTI', 'タイパ', 'LINE', 'サステナブル', 'カフェ巡り'
        ],
        'millennial': [
            # 資産形成 (8)
            'NISA', 'iDeCo', '投資信託', 'FIRE', '株式投資', '仮想通貨', 'S&P500', 'オルカン',
            # キャリア (8)
            '転職', '副業', 'リモートワーク', 'フリーランス', 'リスキリング', 'MBA', 'キャリアアップ', 'スキルアップ',
            # 住宅・生活 (8)
            'マンション購入', '住宅ローン', 'ふるさと納税', 'コストコ', 'IKEA', 'ポイ活', '家計管理', '節約',
            # 育児 (6)
            '保育園', '育児休暇', '子育て支援', '幼児教育', '習い事', 'ワンオペ育児',
            # 健康 (5)
            'ジム', 'ダイエット', 'メンタルヘルス', 'スキンケア', 'ピラティス',
            # エンタメ (5)
        ...
        'genx': [
            ...
            '中学受験', '大学受験', '塾', '教育費', '進学',
            # 健康 (5)
            '人間ドック', 'がん検診', '更年期', '高血圧', '健康診断',
            # 介護 (5)
            '親の介護', '介護保険', '介護施設', 'デイサービス', '介護離職',
            # キャリア・資産 (5)
            '早期退職', '退職金運用', '老後資金', '相続対策', '年金'
        ],
        'boomer': [
            # 年金・退職 (5)
            '年金受給', '退職金', '定年退職', '厚生年金', '国民年金',
            # 健康 (5)
            '高血圧', '糖尿病', '認知症予防', 'ウォーキング', '骨密度',
            # 終活 (5)
            '終活', '遺言書', '相続', 'エンディングノート', '葬儀',
            # 生活 (5)
            '旅行', '趣味', '孫', 'マイナンバー', 'キャッシュレス'
        ],
        'senior': [
            # 介護 (8)
            '介護認定', 'デイサービス', '訪問介護', '介護保険', '要介護', 'ケアプラン', 'ヘルパー', '福祉用具',
            # 施設 (4)
            '特別養護老人ホーム', '有料老人ホーム', 'グループホーム', '老人ホーム',
            # 医療 (4)
            '後期高齢者医療', '認知症', 'リハビリ', '訪問診療',
            # 終末期 (4)
            '看取り', 'ホスピス', '緩和ケア', '延命治療'
        ]
    },
    'kr': {
        'genz': [
            ...
            '원신', '블루아카이브', '발로란트', '리그오브레전드', 'e스포츠', '스타레일',
            'K-POP', '아이돌', 'BTS', 'BLACKPINK', '뉴진스', '최애',
            '무신사', '올리브영', '카카오페이', '네이버페이', '당근마켓',
            '카카오톡', 'MBTI', '갓생', 'MZ세대',
            '취준', '코딩', 'ChatGPT', 'AI', '넷플릭스'
        ],
        'millennial': [
            '주식투자', '부동산', '코인', '재테크', 'ETF', 'ISA', 'S&P500',
            '이직', 'N잡러', '재택근무', '프리랜서', '창업', '리스킬링',
            '아파트', '전세', '청약', '대출', '내집마련',
            '어린이집', '육아휴직', '육아', '워라밸', '맞벌이',
            '헬스', '다이어트', '건강검진', '멘탈케어',
            '넷플릭스', '디즈니플러스', '웹툰', '유튜브 프리미엄'
        ],
        'genx': [
            '입시', '수능', '학원', '교육비', '대학등록금',
            '건강검진', '암검진', '갱년기', '성인병',
