export default {
  async fetch(request, env) {
    ...
    // スコア履歴API
    if (p === '/api/history') return await handleHistory(env, q('country') || 'jp', q('gen') || 'genz', q('kw') || '');
...
// スコア履歴API
async function handleHistory(env, country, genKey, keyword) {
  const key = `gen7_history_${country}_${genKey}`;
  const data = await env.TRENDS_KV.get(key, 'json');
  ...
  if (!data || !data.history) {
    return json({
      ...
      keyword,
      history: [],
      message: '履歴データがまだありません'
    });
  }

  if (keyword) {
    // 特定キーワードの履歴
    const kwHistory = data.history[keyword] || {};
    const historyArray = Object.entries(kwHistory)
    ...
    // 全キーワードの履歴サマリー
    const summary = {};
    for (const [kw, dates] of Object.entries(data.history)) {
      const sortedDates = Object.entries(dates).sort((a, b) => b[0].localeCompare(a[0]));
      if (sortedDates.length >= 2) {
        const latest = sortedDates[0][1];
        const prev = sortedDates[1][1];
        summary[kw] = {
          latest,
          prev,
